...
func (c *Client) CreateAccountGroupTemplate(agt *AccountGroupTemplate) (int64, error) {
	ids, err := c.CreateAccountGroupTemplates([]*AccountGroupTemplate{agt})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountGroupTemplate creates a new account.group.template model and returns its id.
func (c *Client) CreateAccountGroupTemplates(agts []*AccountGroupTemplate) ([]int64, error) {
	var vv []interface{}
	for _, v := range agts {
		vv = append(vv, v)
	}
	return c.Create(AccountGroupTemplateModel, vv)
}
