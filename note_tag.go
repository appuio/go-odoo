...
func (c *Client) CreateNoteTag(nt *NoteTag) (int64, error) {
	ids, err := c.CreateNoteTags([]*NoteTag{nt})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateNoteTag creates a new note.tag model and returns its id.
func (c *Client) CreateNoteTags(nts []*NoteTag) ([]int64, error) {
	var vv []interface{}
	for _, v := range nts {
		vv = append(vv, v)
	}
	return c.Create(NoteTagModel, vv)
}
...
func (c *Client) FindNoteTag(criteria *Criteria) (*NoteTag, error) {
	...
	return nil, fmt.Errorf("note.tag was not found with criteria %v", criteria)
}
...
func (c *Client) FindNoteTagId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("note.tag was not found with criteria %v and options %v", criteria, options)
}
