...
func (c *Client) CreateSnailmailConfirmInvoice(sci *SnailmailConfirmInvoice) (int64, error) {
	ids, err := c.CreateSnailmailConfirmInvoices([]*SnailmailConfirmInvoice{sci})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateSnailmailConfirmInvoice creates a new snailmail.confirm.invoice model and returns its id.
func (c *Client) CreateSnailmailConfirmInvoices(scis []*SnailmailConfirmInvoice) ([]int64, error) {
	var vv []interface{}
	for _, v := range scis {
		vv = append(vv, v)
	}
	return c.Create(SnailmailConfirmInvoiceModel, vv)
}
