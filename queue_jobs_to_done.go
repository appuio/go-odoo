...
func (c *Client) CreateQueueJobsToDone(qjtd *QueueJobsToDone) (int64, error) {
	ids, err := c.CreateQueueJobsToDones([]*QueueJobsToDone{qjtd})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateQueueJobsToDone creates a new queue.jobs.to.done model and returns its id.
func (c *Client) CreateQueueJobsToDones(qjtds []*QueueJobsToDone) ([]int64, error) {
	var vv []interface{}
	for _, v := range qjtds {
		vv = append(vv, v)
	}
	return c.Create(QueueJobsToDoneModel, vv)
}
