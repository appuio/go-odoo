...
func (c *Client) CreateNoteStage(ns *NoteStage) (int64, error) {
	ids, err := c.CreateNoteStages([]*NoteStage{ns})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateNoteStage creates a new note.stage model and returns its id.
func (c *Client) CreateNoteStages(nss []*NoteStage) ([]int64, error) {
	var vv []interface{}
	for _, v := range nss {
		vv = append(vv, v)
	}
	return c.Create(NoteStageModel, vv)
}
...
func (c *Client) FindNoteStage(criteria *Criteria) (*NoteStage, error) {
	...
	return nil, fmt.Errorf("note.stage was not found with criteria %v", criteria)
}
...
func (c *Client) FindNoteStageId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("note.stage was not found with criteria %v and options %v", criteria, options)
}
