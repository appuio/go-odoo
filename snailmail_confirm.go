...
func (c *Client) CreateSnailmailConfirm(sc *SnailmailConfirm) (int64, error) {
	ids, err := c.CreateSnailmailConfirms([]*SnailmailConfirm{sc})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateSnailmailConfirm creates a new snailmail.confirm model and returns its id.
func (c *Client) CreateSnailmailConfirms(scs []*SnailmailConfirm) ([]int64, error) {
	var vv []interface{}
	for _, v := range scs {
		vv = append(vv, v)
	}
	return c.Create(SnailmailConfirmModel, vv)
}
