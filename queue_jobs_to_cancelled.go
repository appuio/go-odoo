...
func (c *Client) CreateQueueJobsToCancelled(qjtc *QueueJobsToCancelled) (int64, error) {
	ids, err := c.CreateQueueJobsToCancelleds([]*QueueJobsToCancelled{qjtc})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateQueueJobsToCancelled creates a new queue.jobs.to.cancelled model and returns its id.
func (c *Client) CreateQueueJobsToCancelleds(qjtcs []*QueueJobsToCancelled) ([]int64, error) {
	var vv []interface{}
	for _, v := range qjtcs {
		vv = append(vv, v)
	}
	return c.Create(QueueJobsToCancelledModel, vv)
}
