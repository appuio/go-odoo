...
func (c *Client) CreateNoteNote(nn *NoteNote) (int64, error) {
	ids, err := c.CreateNoteNotes([]*NoteNote{nn})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateNoteNote creates a new note.note model and returns its id.
func (c *Client) CreateNoteNotes(nns []*NoteNote) ([]int64, error) {
	var vv []interface{}
	for _, v := range nns {
		vv = append(vv, v)
	}
	return c.Create(NoteNoteModel, vv)
}
...
func (c *Client) FindNoteNote(criteria *Criteria) (*NoteNote, error) {
	...
	return nil, fmt.Errorf("note.note was not found with criteria %v", criteria)
}
...
func (c *Client) FindNoteNoteId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("note.note was not found with criteria %v and options %v", criteria, options)
}
